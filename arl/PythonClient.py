...
class AirSimClient:
        ...
        def moveToPosition(self, x, y, z, velocity, drivetrain, yaw_mode, lookahead, adaptive_lookahead):
        ...
        def toEulerianAngle(self, q):
            ...
            if (t2 < -1.0):
                t2 = -1.0
            pitch = math.asin(t2) # THIS SHOULD BE *ASIN* ?!--------------------------------------------------------------?!
        ...
        def toQuaternion(self, eul):

            t0 = math.cos(eul[2] * 0.5)
            t1 = math.sin(eul[2] * 0.5)
            t2 = math.cos(eul[0] * 0.5)
            t3 = math.sin(eul[0] * 0.5)
            t4 = math.cos(eul[1] * 0.5)
            t5 = math.sin(eul[1] * 0.5)
